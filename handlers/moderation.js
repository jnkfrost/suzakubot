...
// Rileva "cundo" scritto in qualsiasi modo
function contieneCundo(text) {
    // Normalizza il testo: rimuove accenti, simboli, spazi e lo rende minuscolo
    const clean = text
        .normalize("NFD")
    ...
    // Crea una regex che permette lettere e simboli tra le lettere di "cundo"
    // Es: c.u-n d o, C U N D O, ecc.
    const pattern = /c\s*[\W_]*\s*u\s*[\W_]*\s*n\s*[\W_]*\s*d\s*[\W_]*\s*o/;

    return pattern.test(clean);
}
const paroleBandite = [
  'puttana', 'frocio', 'negro', 'ricchione', 'sborra', 'leccare i piedi',
  'succhiare i cazzi', 'zitto gay', 'ritardato', 'mongoloide', 'svegliati freddo',
  'tumore', 'cancro', 'succhiamelo', 'dislessico di merda', 'discalculi', 'down'
];

// Crea una regex flessibile per ogni parola bandita
function buildBannedWordRegex(word) {
    // Escape dei caratteri speciali
    const escaped = word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    // Permetti simboli/spazi tra le lettere
    const pattern = escaped.split('').join('[\\W_]*');
    // Word boundaries solo se la parola è composta da lettere/numeri
    return new RegExp(`\\b${pattern}\\b`, 'i');
}

// Precompila le regex per tutte le parole bandite
const bannedRegexList = paroleBandite.map(buildBannedWordRegex);

function contieneParolaBandita(text) {
    const clean = text.normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "")
        .toLowerCase();
    return bannedRegexList.some(regex => regex.test(clean));
}

const paroleContestuali = [
  { parola: 'trans', regex: /\btrans\b.*(di merda|schifo|fa schifo|puttana|frocio|ricchione)/i },
  { parola: 'cappella', regex: /\bcappella\b.*(cazzo|pene|sesso|scopare)/i }
];

function contieneParolaContestuale(text) {
  return paroleContestuali.some(entry => entry.regex.test(text));
}

async function handleModeration(message) {
  if (
    contieneParolaBandita(message.body) ||
    contieneParolaContestuale(message.body) ||
    filtroBestemmie(message.body)
  ) {
    await message.delete(true);
    await message.reply(
      `★彡[ MODERAZIONE ]彡★
✖️ Messaggio cancellato!
⚠️ Linguaggio offensivo rilevato

Per favore, rispetta tutti i membri del gruppo.`
    );
    return true; // Indica che il messaggio è stato moderato
  }
  return false; // Nessuna moderazione necessaria
}
